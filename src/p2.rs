...
fn main() {
    ...
    let immune_groups : Vec<Group> = lines_to_groups(immune_lines, Army::Immune);
    ...
    let infection_groups : Vec<Group> = lines_to_groups(infection_lines, Army::Infection);
    
    let mut lo : i32 = 0;
    let mut hi : i32 = 1000; 
    let mut boost : i32 = (lo + hi) / 2;

    // Binary Search for answer
    while lo <= hi {
        boost = (lo + hi) / 2;
        let (immune, infection) = sol(&immune_groups, &infection_groups, boost);

        if immune != 0 && infection == 0 { 
            hi = boost - 1;
        } else if immune != 0 && infection != 0 { 
            lo = boost + 1;
        } else if immune != 0 {
            hi = boost - 1;
        } else if immune == 0 {
    ...
    let (immune, infection) = sol(&immune_groups, &infection_groups, boost);
    println!("Immune: {}, Infection: {}", immune, infection);

    let (immune, infection) = sol(&immune_groups, &infection_groups, boost + 1);
    println!("Immune: {}, Infection: {}", immune, infection);
    println!("Ans: {}", immune);
}

fn sol(immune_groups : &Vec<Group>, infection_groups : &Vec<Group>, boost : i32) -> (i32, i32) {
    
    let mut temp : Vec<Group> = Vec::new(); 

    for group in immune_groups {
        temp.push(Group {
            attack: Attack {damage: group.attack.damage + boost, kind: group.attack.kind}, 
            .. *group
        });
    }

    let mut immune_groups = temp;
    let mut infection_groups = infection_groups.clone();
    ...
    let mut prev : Vec<i32> = Vec::from([-1, -1]);
    loop {
        ...
        let immune_count = immune_groups.iter().fold(0, |acc, x| acc + x.units);
        let infection_count = infection_groups.iter().fold(0, |acc, x| acc + x.units);

        if immune_count == prev[0 as usize] && infection_count == prev[1 as usize] {
            break;
        }
        prev[0] = immune_count;
        prev[1] = infection_count;
    ...
    (immune_groups.iter().fold(0, |acc, x| acc + x.units), infection_groups.iter().fold(0, |acc, x| acc + x.units))
}
